...
/***template***/
ll ara[100009], cumsum[100009], cumsum2[100009];
ll state[100009];
int main()
{
    ll n, k;
    ...
    for(ll i = 1; i <= n; i++) {
        ...
        cumsum[i] += cumsum[i - 1];
        cumsum2[i] = ara[i] + cumsum2[i - 1];
    }
    ll ans = INT_MIN;
    for(ll i = 1; i <= n - k + 1; i++) {
        ll tmp1 = cumsum[i - 1];
        ll tmp2 = cumsum2[i + k - 1] - cumsum2[i - 1];
        ll tmp3 = cumsum[n] - cumsum[i + k - 1];
        ll sum = tmp1 + tmp2 + tmp3;
        ans = max(sum, ans);
    }
    cout << ans << endl;
    return 0;
